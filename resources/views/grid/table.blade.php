            ...
            @foreach($grid->rows() as $row)
                @if(get_class($grid->model()->path()->getOriginalModel()) === "App\Models\NewsItem")
                    <tr {!! $row->getRowAttributes() !!}>
                        ...
                        @foreach($grid->visibleColumnNames() as $name)
                            @if($name !== '__actions__')
                                <td onclick="window.location.assign('news-items/'+{{$row->column('id')}}+'/edit')" {!! $row->getColumnAttributes($name) !!}>
                                    {!! $row->column($name) !!}
                                </td>
                            @else
                ...
                @elseif(get_class($grid->model()->path()->getOriginalModel()) === "App\Models\Quiz")
                    <tr {!! $row->getRowAttributes() !!}>
                        @foreach($grid->visibleColumnNames() as $name)
                            @if($name !== '__actions__')
                                <td onclick="window.location.assign('quizzes/'+{{$row->column('id')}}+'/edit')" {!! $row->getColumnAttributes($name) !!}>
                                    {!! $row->column($name) !!}
                                </td>
                            @else
                                <td {!! $row->getColumnAttributes($name) !!}>
